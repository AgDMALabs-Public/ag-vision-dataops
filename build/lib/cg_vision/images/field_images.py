...
from ..data_io import aws_io, local_io, databricks_io
from . import image_quality as iq
from ..data_io import schemas as sch
import logging
...
class FieldImage:
    def __init__(self, platform: str = None, cloud_bucket: str = None, img_key: str = None,
        ...
        if not (isinstance(metadata, Image) or metadata is None):
    ...
    def read_img(self):
        if self.platform == 'aws':
            try:
                self.image = aws_io.read_image_from_s3(bucket_name=self.cloud_bucket,
                                                       key=self.img_key)
            except Exception as e:
                self.image = np.ndarray([])
                logger.warning(f'Exception occurred while reading image from S3: {e}')
            ...
            logger.warning(f'The cloud platform need to be aws or db')

    def read_metadata(self):
        if self.platform == 'aws':
            try:
                m_data = aws_io.read_json_from_s3(bucket_name=self.cloud_bucket,
                                                  object_key=self.metadata_key)
                self.metadata = Image(**m_data)
            except Exception as e:
                self.metadata = None
                logger.warning(f'Exception occurred while reading metadata from S3: {e}')
            ...
            logger.warning(f'The cloud platform need to be local, aws, or db')
    ...
    def save_img(self):
        if self.image and self.cloud_bucket and self.img_key:
            if self.platform == 'aws':
                try:
                    aws_io.save_image_to_s3(image=self.image,
                                            bucket_name=self.cloud_bucket,
                                            key=self.img_key)
                ...
                logger.warning(f'The cloud platform need to be aws')

        else:
            logger.warning(f'One of the following attributes is None: image: {self.image},'
                           f' cloud_bucket: {self.cloud_bucket}, cloud_key: {self.cloud_img_key}')

    def save_metadata(self):
        if self.metadata and self.cloud_bucket and self.metadata_key:
            if self.platform == 'aws':
                try:
                    if isinstance(self.metadata, Image):
                        json_str = self.metadata.dump_json()
                        aws_io.save_json_to_s3(json_data=json_str,
                                               bucket_name=self.cloud_bucket,
                                               key=self.metadata_key)
                    else:
                        logger.warning(f'Trying to save {self.metadata} to S3, but it is not a dictionary.')
                except Exception as e:
                    logger.warning(f'Exception occurred while saving metadata to S3: {e}')

            elif self.platform == 'db':
                try:
                    if isinstance(self.metadata, Image):
                        databricks_io.save_json_to_databricks(data=self.metadata.dump_json(),
                ...
                try:
                    if isinstance(self.metadata, Image):
                        local_io.save_json(data=self.metadata.dump_json(),
                                           file_path=self.metadata_key)
                except Exception as e:
                    logger.warning(f'Exception occurred while saving metadata to local: {e}')

        else:
            logger.warning(f'One of the following attributes is None: metadata: {self.metadata},'
                           f' cloud_bucket: {self.cloud_bucket}, cloud_metadata_key: {self.metadata_key}')
    ...
    def add_flat_data_to_metadata(self, key: str, value: str, delimiter: str = ':', overwrite: bool = False):
        self.metadata = sch.add_flat_schema_data_to_metadata(metadata_dict=self.metadata,
                                                             flat_key=key,
                                                             val=value,
                                                             delimiter=delimiter,
                                                             overwrite=overwrite)
    ...
    # TODO this needs to be fixed
    def add_note_to_metadata(self, msg: str, author: str):
        self.metadata = sch.add_notes_to_metadata(metadata_dict=self.metadata,
                                                  msg=msg,
                                                  author=author)
